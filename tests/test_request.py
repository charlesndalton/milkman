...
def test_request_swap(cow_anywhere, user, gno_whale, gno, dai, univ2_price_checker):
    ...
    encoded_market_order = encode_abi(['address', 'address', 'address', 'address', 'uint256', 'address', 'uint256'], [user.address, user.address, gno.address, dai.address, int(amount), univ2_price_checker.address, 0])
