...
contract CurvePriceChecker is IPriceChecker {
    ...
    IAddressProvider internal constant ADDRESS_PROVIDER = IAddressProvider(0x0000000022D53366457F9d5E68Ec105046FC4383);
    ...
    function checkPrice(
        ...
        { // block scoping to prevent stack too deep
            address[8] memory _tokensInPool = registry.get_underlying_coins(_pool);
            ...
            require(_i != 0 || _j != 0); // dev: something went wrong, probably caused by this not being a normal pool but being labeled as one
        }            

        uint256 _expectedOut = ICurvePool(_pool).get_dy_underlying(_i, _j, _amountIn); 
        
        // within max slippage on either side
        if (_minOut > _expectedOut.mul(MAX_BPS.sub(_maxSlippage)).div(MAX_BPS) && _minOut < _expectedOut.mul(MAX_BPS.add(_maxSlippage)).div(MAX_BPS)) {
            _isPriceGood = true;
        }
    }
}
