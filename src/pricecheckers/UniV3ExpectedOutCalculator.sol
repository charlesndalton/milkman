...
contract UniV3ExpectedOutCalculator is IExpectedOutCalculator {
    ...
    IUniswapV3StaticQuoter internal constant QUOTER = IUniswapV3StaticQuoter(0x7637Aaeb5BD58269B782726680d83f72C651aE74);
    ...
    function getExpectedOut(uint256 _amountIn, address _fromToken, address _toToken, bytes calldata _data)
        ...
        returns (uint256)
    {
        (address[] memory _swapPath, uint24[] memory _poolFees) = abi.decode(_data, (address[], uint24[]));

        return _getExpectedOut(_amountIn, _swapPath, _poolFees);
    }

    function _getExpectedOut(uint256 _amountIn, address[] memory _swapPath, uint24[] memory _poolFees)
        internal
        view
        returns (uint256)
    {
        ...
        for (uint256 _i = 0; _i < _poolFees.length; _i++) {
            ...
            _path = abi.encodePacked(_path, uint24(uint256(_poolFees[_i]).mul(100)), _swapPath[_i.add(1)]);
        }
